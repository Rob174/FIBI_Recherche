...
//#define DEBUG_LV1
//#define DEBUG_LV2
//#define DEBUG_POSTOUR
#define EPSILON 1e-5
void run_BI_NL(Tour* tour, DistanceMatrix* dist, Result* res, Config* conf) {
	...
	while (true) {
		...
		*/
		...
		for (int i = 0; i <= conf->NUM_POINTS-1; i++) {
			...
			*/
			// tour-> at -> return town id
			int n_prime = dist_ordered->rank_of_edge(tour->at(cyclic_id(i), conf), tour->at(cyclic_id(i + 1),conf))-1; // rank to stop
			for (int jprime = 1; jprime <= n_prime-1; jprime++) {
				int j = posOfTownInTour[
						dist_ordered->town_of_rank(tour->at(cyclic_id(i),conf),cyclic_id(jprime))
				];
				if (i == j)
					continue;
#if defined(DEBUG_LV1)
				if (j < 0 || j >= conf->NUM_POINTS)
					exit(1);
#endif
				double delta_ij =
					dist->get(
		...
		// Inequality 2: cost(TJ,TJ+1) > cost(TI,TJ+1)
		for (int i = 1; i <= conf->NUM_POINTS; i++) {
			int n_prime = dist_ordered->rank_of_edge(tour->at(cyclic_id(i),conf), tour->at(cyclic_id(i - 1),conf)) - 1;
			for (int jprime = 1; jprime <= n_prime-1; jprime++) {
				int j = posOfTownInTour[
						dist_ordered->town_of_rank(tour->at(cyclic_id(i),conf),cyclic_id(jprime))
				];
...
#if defined(DEBUG_POSTOUR)
		for (int a = 0; a < conf->NUM_POINTS; a++) {
			std::cout << posOfTownInTour[a] << ",";
		}
		std::cout << std::endl;
#endif
#if defined(DEBUG_LV1) || defined(DEBUG_LV2)
		...
		std::set<int> seen;
		for (int a = 0; a < conf->NUM_POINTS; a++) {
			if (posOfTownInTour[a] < 0 || posOfTownInTour[a] >= conf->NUM_POINTS)
				exit(1);
			if (seen.find(posOfTownInTour[a]) != seen.end())
				exit(1);
		}
#endif
		// Update cost
