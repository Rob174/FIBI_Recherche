...
void test_improve_maxsat()
{
	...
	for (auto it = j.begin(); it != j.end(); ++it)
	{
		...
		AbstractContainerMAXSAT *cont = new GREEDYContainer();

		unique_ptr<vector<var_assignment_t>> assignment(improve_maxsat(var_to_clauses, *clauses, *weights, n_vars, c.SEED_ASSIGN.get(), *cont));
		delete cont;
