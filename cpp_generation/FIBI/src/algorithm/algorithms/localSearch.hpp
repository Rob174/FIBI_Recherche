...
* */
template <typename T_Swap, typename T_Container, typename T_Neighbourhood, typename T_Config, typename T_Flip, bool debug = false, const int num_iter_max = -1>
class LocalSearch {
private:
...
public:
	...
	* 	*/
	void run(T_Container& c, const T_Config& cf, T_Flip& f) {
		...
		while (!stop_loop)
		{
			stop_loop = n.explore_flips(c, cf, f);
			num_iter++;
			if constexpr (debug) {
				c.print();
			}
