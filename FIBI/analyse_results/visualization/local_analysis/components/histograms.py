...
from FIBI.analyse_results.visualization.utils import default_fibi_order, dicos_fibi_diff, dicos_fibi_diff_with_seed_glob
...
class HistoFIBIDiff:
    ...
    def __call__(self, keys: dict, dfs: List[DfExtract]) -> HTMLDataOut:
        dico_fibi,dico_seeds = dicos_fibi_diff_with_seed_glob(dfs, self.metric, diff=True)
        # make a df with the difference and the seed_glob pairs
        df = pd.DataFrame({'diff':dico_fibi['-'.join(default_fibi_order())],'SEED_GLOB':dico_seeds['-'.join(default_fibi_order())]})
        fig = go.Figure()
        ...
        for binStart,binEnd in zip(bin_edges[:-1],bin_edges[1:]):
            groups[f'{binStart},{binEnd}'] = list(df[(df['diff']>=binStart) & (df['diff']<binEnd)]['SEED_GLOB'].values)
            groups[f'{binStart},{binEnd}'] = f'{binStart}->{binEnd}:'+';'.join(groups[f'{binStart},{binEnd}'][:min(5,len(groups[f'{binStart},{binEnd}']))])
        fig.add_trace(
            go.Histogram(
